...
// Rec #6 Layout: [Search bar] [Online Friends Row] [Pinned] [Conversation list]
// Video Calls launched from 📹 icon in chat header (Rec #16)

import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';

// ── Mock online friends (real: Firestore presence) ──────────────────────────
...
// ── Chat Thread (inline simplified view) ────────────────────────────────────
function ChatThread({ conv, onBack, onVideoCall }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', background:'#0a0a18' }}>
      {/* Chat header */}
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 16px', background:'rgba(15,12,41,0.95)', borderBottom:'1px solid rgba(255,255,255,0.08)', backdropFilter:'blur(20px)' }}>
        <button onClick={onBack} style={{ color:'#818cf8', fontSize:22, fontWeight:800, lineHeight:1 }}>‹</button>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontSize:11, color: conv.online ? '#10b981' : '#64748b' }}>{conv.online ? 'Online now' : 'Offline'}</div>
        </div>
        {/* Rec #16: 📹 Video Call launched from chat header */}
        <button onClick={onVideoCall} style={{ padding:'8px 12px', background:'rgba(99,102,241,0.18)', border:'1px solid rgba(99,102,241,0.35)', borderRadius:10, color:'#818cf8', fontSize:18, minWidth:44, minHeight:44, display:'flex', alignItems:'center', justifyContent:'center' }} title="Start video call">📹</button>
        <button style={{ padding:'8px', color:'#64748b', fontSize:18, minWidth:44, minHeight:44, display:'flex', alignItems:'center', justifyContent:'center' }}>•••</button>
      </div>
      ...
      {/* Input */}
      <div style={{ padding:'12px 16px', borderTop:'1px solid rgba(255,255,255,0.07)', display:'flex', gap:10, alignItems:'center', background:'rgba(15,12,41,0.95)', backdropFilter:'blur(20px)' }}>
        <input value={msg} onChange={e=>setMsg(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder="Message…" style={{ flex:1, background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:24, padding:'10px 16px', color:'#f1f5f9', fontSize:15, outline:'none' }} />
        <button onClick={send} style={{ width:44, height:44, borderRadius:'50%', background:'linear-gradient(135deg,#6366f1,#ec4899)', border:'none', color:'white', fontSize:18, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>➤</button>
      </div>
    </div>
  );
}
...
export default function MessagesPage() {
  ...
  const filtered  = search.trim()
    ? allConvs.filter(c => c.name.toLowerCase().includes(search.toLowerCase()))
    : null;
  ...
  return (
    <div style={{ background:'#0a0a18', minHeight:'100vh', paddingBottom:80 }}>
        ...
        <div style={{ borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
          ...
          <div style={{ display:'flex', gap:14, overflowX:'auto', padding:'0 16px 14px', scrollbarWidth:'none' }}>
            {ONLINE_FRIENDS.filter(f => f.online).map(f => (
              <div key={f.id} onClick={() => setActive({ id: f.id, name: f.name, last: 'Say hi!', time: 'now', unread:0, emoji: f.emoji, color: f.color, online: true })} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, cursor:'pointer', flexShrink:0 }}>
                <div style={{ position:'relative' }}>
                  <div style={{ width:50, height:50, borderRadius:'50%', background:f.color, display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, border:'2.5px solid transparent', backgroundClip:'padding-box', outline:`2.5px solid ${f.color}55`, outlineOffset:2 }}>{f.emoji}</div>
      ...
      {filtered && filtered.length > 0 && (
        <div>
          {filtered.map(c => <ConvRow key={c.id} conv={c} onClick={() => setActive(c)} />)}
        </div>
      )}
