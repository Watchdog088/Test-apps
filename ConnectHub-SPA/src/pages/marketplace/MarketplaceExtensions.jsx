 ...
 *   allItems    — Array of all current Browse listings
 *   currentItem — the item currently being viewed
 *   onAddBundle — (items: Array, discountPct: number) => void
 *   onClose
 *
 * Usage in product detail (below seller info):
 *   {showBundleModal && <BundleDiscountModal seller={item.seller}
...
export function BundleDiscountModal({ seller, allItems = [], currentItem, onAddBundle, onClose }) {
  ...
  return (
    <Backdrop onClose={onClose} maxH="90vh">
      ...
      {/* Current item (locked) */}
      <div style={{ fontSize:'12px', fontWeight:700, color:DARK.muted, marginBottom:'8px' }}>IN YOUR BUNDLE</div>
      <div style={{ display:'flex', alignItems:'center', gap:'12px', background:DARK.bg0,
        borderRadius:'12px', padding:'10px', marginBottom:'4px', border:`1px solid ${DARK.accent}` }}>
        <div style={{ width:'44px', height:'44px', borderRadius:'10px',
          background:currentItem?.color||DARK.bg2, display:'flex', alignItems:'center',
          justifyContent:'center', fontSize:'22px', flexShrink:0 }}>
          {currentItem?.emoji||'🛍️'}
...
/* ─────────────────────────────────────────────────────────────────────────── */
/**
 * Horizontal scroll strip of similar listings shown in the product detail panel.
 *
 * Props:
 *   currentItem — { id, category, price, condition }
 *   allItems    — all Browse listings
 *   onSelect    — (item) => void  — opens that item's detail
 *
 * Usage in product detail (below description, above seller info):
...
export function SimilarItemsRow({ currentItem, allItems = [], onSelect }) {
  ...
  return (
    <div style={{ marginBottom:'20px' }}>
      ...
      <div role="list" style={{ display:'flex', gap:'10px', overflowX:'auto', paddingBottom:'6px' }}>
        {similar.map(item => (
          <button key={item.id} onClick={() => onSelect?.(item)}
            role="listitem"
            aria-label={`View ${item.title} for $${item.price}`}
            style={{ flexShrink:0, width:'120px', background:DARK.bg0, borderRadius:'14px',
              border:`1px solid ${DARK.bg2}`, padding:'0 0 10px 0', cursor:'pointer', textAlign:'left',
              overflow:'hidden' }}>
            <div style={{ width:'100%', height:'80px', background:item.color||DARK.bg2,
...
/* ─────────────────────────────────────────────────────────────────────────── */
/**
 * Generates a QR code for the listing URL using the Google Charts QR API.
 * No npm package required; falls back to URL text display if offline.
 *
 * Props: item — { id, title, price, emoji }, onClose
 *
 * Usage in product detail header bar (beside Share button):
 *   {showQRModal && <QRCodeModal item={selectedItem} onClose={() => setShowQRModal(false)} />}
 */
export function QRCodeModal({ item, onClose }) {
  const { show, Toast } = useToast();
  ...
  async function handleDownload() {
    try {
      ...
      show('⚠️ Download failed — right-click the QR image to save it');
    }
  }
  ...
  return (
    <Backdrop onClose={onClose}>
      ...
      <div style={{ display:'flex', justifyContent:'center', marginBottom:'16px' }}>
        <div style={{ background:'white', padding:'16px', borderRadius:'16px',
          ...
          {imgError ? (
            <div style={{ width:'200px', height:'200px', display:'flex', flexDirection:'column',
              alignItems:'center', justifyContent:'center', gap:'10px',
              border:'2px dashed #e2e8f0', borderRadius:'8px', padding:'16px', textAlign:'center' }}>
              <span style={{ fontSize:'40px' }}>🔗</span>
              <div style={{ fontSize:'9px', color:'#475569', wordBreak:'break-all', lineHeight:1.4 }}>
                {listingUrl}
              </div>
            </div>
          ) : (
      ...
      {/* Caption */}
      <div style={{ textAlign:'center', fontSize:'13px', color:DARK.muted, marginBottom:'20px', lineHeight:1.6 }}>
        Scan to open this listing on any device.
        <div style={{ fontSize:'11px', color:DARK.dim, marginTop:'4px' }}>
          Works in-store, at meetups, or on printed flyers.
        </div>
      </div>
